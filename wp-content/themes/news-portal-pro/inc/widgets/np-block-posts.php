...
class News_Portal_Block_Posts extends WP_widget {
    ...
    public function __construct() {
        $widget_ops = array( 
            ...
            'description' => __( 'Displays block posts from selected category in different layouts.', 'news-portal-pro' )
        );
        parent::__construct( 'news_portal_block_posts', __( 'NP: Block Posts', 'news-portal-pro' ), $widget_ops );
    }
    ...
    private function widget_fields() {
        ...
        $np_categories_dropdown = news_portal_categories_dropdown();
        ...
        $fields = array(
            ...
            'block_cat_id' => array(
                ...
                'news_portal_widgets_title'        => __( 'Block Category', 'news-portal-pro' ),
                'news_portal_widgets_field_type'   => 'select',
                'news_portal_widgets_default'      => '0',
                'news_portal_widgets_field_options' => $np_categories_dropdown
            ),
    ...
    public function widget( $args, $instance ) {
        ...
        $np_block_cat_id   = empty( $instance['block_cat_id'] ) ? '' : $instance['block_cat_id'];
        $np_block_layout   = empty( $instance['block_layout'] ) ? 'layout1' : $instance['block_layout'];

        $widget_title_args = array(
                'title'  => $np_block_title,
                'cat_id' => $np_block_cat_id
            );
        ...
        <div class="np-block-wrapper block-posts np-clearfix <?php echo esc_attr( $np_block_layout ); ?>">
            <?php 
                if ( !empty( $np_block_title ) ) {
                    do_action( 'np_widget_title', $widget_title_args );
                }
            ?>
            <div class="np-block-posts-wrapper">
            	<?php
            		switch ( $np_block_layout ) {
            			case 'layout2':
            				news_portal_block_second_layout_section( $np_block_cat_id );
            				break;

            			case 'layout3':
            				news_portal_block_box_layout_section( $np_block_cat_id );
            				break;

            			case 'layout4':
            				news_portal_block_alternate_grid_section( $np_block_cat_id );
            				break;
            			
            			default:
            				news_portal_block_default_layout_section( $np_block_cat_id );
            				break;
            		}
            	?>
