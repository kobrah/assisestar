...
class News_Portal_Featured_Posts extends WP_widget {
    ...
    public function __construct() {
        $widget_ops = array( 
            ...
            'description' => __( 'Displays featured posts from selected categories in different layouts.', 'news-portal-pro' )
        );
        parent::__construct( 'news_portal_featured_posts', __( 'NP: Featured Posts', 'news-portal-pro' ), $widget_ops );
    }
        ...
        $fields = array(
            ...
            'block_layout' => array(
                ...
                'news_portal_widgets_field_options' => array(
                    'layout1' => esc_url( get_template_directory_uri() . '/assets/images/featured-post-1.png' ),
                    'layout2' => esc_url( get_template_directory_uri() . '/assets/images/featured-post-2.png' )
                )
            ),
    ...
    public function widget( $args, $instance ) {
        ...
        echo $before_widget;
    ?>
        <div class="np-block-wrapper featured-posts np-clearfix <?php echo esc_attr( $np_block_layout ); ?>">
            <?php 
                ...
                <?php
                    if ( !empty( $np_block_cat_ids ) ) {
                        ...
                        switch ( $np_block_layout ) {
                            case 'layout2':
                                news_portal_featured_posts_layout_two( $np_posts_args, $cats_list_option );
                                break;
                            
                            default:
                                news_portal_featured_posts_layout_one( $np_posts_args );
                                break;
                        }
                    }
                ?>
