...
class News_Portal_Slider extends WP_widget {
    ...
    public function __construct() {
        $widget_ops = array( 
            'classname' => 'news_portal_slider',
            'description' => __( 'Displays posts from selected categories in slider section.', 'news-portal-pro' )
        );
        parent::__construct( 'news_portal_slider', __( 'NP: Slider', 'news-portal-pro' ), $widget_ops );
    }
    ...
    private function widget_fields() {
        ...
        $fields = array(
            ...
            'slider_layout' => array(
                'news_portal_widgets_name'         => 'slider_layout',
                'news_portal_widgets_title'        => __( 'Slider Layouts', 'news-portal-pro' ),
                'news_portal_widgets_default'      => 'layout1',
                'news_portal_widgets_field_type'   => 'selector',
                'news_portal_widgets_field_options' => array(
                    'layout1' => esc_url( get_template_directory_uri() . '/assets/images/block-layout3.png' ),
                    'layout2' => esc_url( get_template_directory_uri() . '/assets/images/np-slider-2.png' )
                )
            ),
            ...
            'slider_shortcode' => array(
                'news_portal_widgets_name'         => 'slider_shortcode',
                'news_portal_widgets_title'        => __( 'Slider Shortcode', 'news-portal-pro' ),
                'news_portal_widgets_description'  => __( 'Use third party slider shortcodes (like revolution slider).', 'news-portal-pro' ),
    ...
    public function widget( $args, $instance ) {
        ...
        $np_slider_layout   = empty( $instance['slider_layout'] ) ? 'layout1' : $instance['slider_layout'];
        $np_slider_code     = empty( $instance['slider_shortcode'] ) ? '' : $instance['slider_shortcode'];
        ...
        echo $before_widget;
    ?>
    	<div class="np-block-wrapper slider-posts np-slider np-clearfix <?php echo 'slider-'. esc_attr( $np_slider_layout ); ?>" data-layout="<?php echo esc_attr( $np_slider_layout ); ?>" data-auto="<?php echo esc_attr( $slide_auto ); ?>" data-control="<?php echo esc_attr( $slide_control ); ?>" data-pager="<?php echo esc_attr( $slide_pager ); ?>" data-speed="<?php echo absint( $np_slide_speed ); ?>" data-pause="<?php echo absint( $np_slide_pause ); ?>">
    		<?php                
                ...
                switch ( $np_slider_layout ) {
                    case 'layout2':
                        news_portal_slider_layout_two( $np_slider_args, $cats_list_option );
                        break;
                    
                    default:
                        news_portal_slider_layout_one( $np_slider_args, $cats_list_option );
                        break;
                }
    		?>
